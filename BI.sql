...
SELECT TOP 3 PAGO_SUCURSAL, SUM(PAGO_TOTAL) AS IMPORTE_TOTAL_DE_CUOTAS, MP_DETALLE, TIEMPO_MES, TIEMPO_ANIO FROM LOREM_IPSUM.BI_H_PAGOS
...
SELECT RANGO_ETARIO_DESCRIPCION, AVG(PAGO_TOTAL/PAGO_CUOTAS) AS PROMEDIO FROM LOREM_IPSUM.BI_H_PAGOS
...
SELECT 
    MP_DETALLE, 
    CAST(CAST(ROUND((SUM(PAGO_DESCUENTO) * 100.0 / NULLIF((SUM(PAGO_TOTAL) + SUM(PAGO_DESCUENTO)), 0)), 2) AS DECIMAL(10,2)) as varchar(10)) + '%' AS PORCENTAJE_DESCUENTO_APLICADO 
FROM 
    LOREM_IPSUM.BI_H_PAGOS
JOIN 
